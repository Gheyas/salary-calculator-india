...
class RetirementCalculator {
    ...
    displayResults() {
        ...
        document.getElementById('finalBasicPay').textContent = `â‚¹${finalResult.basicPay.toLocaleString()}`;
        document.getElementById('finalDAAmount').textContent = `â‚¹${finalResult.daAmount.toLocaleString()}`;
        document.getElementById('finalGrossSalary').textContent = `â‚¹${finalResult.grossSalary.toLocaleString()}`;
        ...
        this.results.forEach(result => {
            ...
            row.innerHTML = `
                ...
                <td class="border border-gray-300 px-4 py-2 text-right">â‚¹${result.basicPay.toLocaleString()}</td>
                <td class="border border-gray-300 px-4 py-2 text-right">${result.daPercent.toFixed(1)}%</td>
                <td class="border border-gray-300 px-4 py-2 text-right">â‚¹${result.daAmount.toLocaleString()}</td>
                <td class="border border-gray-300 px-4 py-2 text-right">â‚¹${result.hraAmount.toLocaleString()}</td>
                <td class="border border-gray-300 px-4 py-2 text-right font-semibold">â‚¹${result.grossSalary.toLocaleString()}</td>
            `;
            ...
            if (result.isTransition) {
                ...
                noteRow.innerHTML = `
                    <td colspan="7" class="border border-gray-300 px-4 py-2 text-sm text-yellow-700 italic">
                        <strong>Note:</strong> Fitment factor applied: â‚¹${result.oldBasicPay.toLocaleString()} Ã— ${result.fitmentFactor} = â‚¹${result.basicPay.toLocaleString()}. DA has been reset to 0% after pay commission transition.
                    </td>
                `;
    ...
    downloadPDF() {
        ...
        doc.setFontSize(10);
        doc.text(`Current Basic Pay: â‚¹${document.getElementById('basicPay').value}`, 20, 75);
        ...
        doc.setFontSize(10);
        doc.text(`Basic Pay: â‚¹${finalResult.basicPay.toLocaleString()}`, 20, 140);
        doc.text(`DA Amount: â‚¹${finalResult.daAmount.toLocaleString()}`, 20, 150);
        doc.text(`Gross Salary: â‚¹${finalResult.grossSalary.toLocaleString()}`, 20, 160);
...
class RetirementBenefitsCalculator {
    ...
    calculateNPS(existingCorpus, returnRate, annuityRate) {
        ...
        let totalInvested = existingCorpus;  // Start with existing corpus
        const monthlyReturn = returnRate / 12 / 100;
        const currentYear = new Date().getFullYear();

        this.salaryResults.forEach(yearData => {
            if (yearData.year >= currentYear) {
                const grossSalary = yearData.grossSalary;  // This is MONTHLY gross salary
                const monthlyContribution = grossSalary * 0.24;  // 24% of monthly salary
                const annualContribution = grossSalary * 0.24 * 12;  // Annual contribution
    ...
    displayNPSResults() {
        ...
        scenarios.forEach((scenario) => {
            ...
            document.getElementById(`${scenario}Invested`).textContent = `â‚¹${this.formatCurrency(result.totalInvested)}`;
            document.getElementById(`${scenario}Corpus`).textContent = `â‚¹${this.formatCurrency(result.finalCorpus)}`;
            document.getElementById(`${scenario}LumpSum`).textContent = `â‚¹${this.formatCurrency(result.lumpSum)}`;
            document.getElementById(`${scenario}Pension`).textContent = `â‚¹${this.formatCurrency(result.monthlyPension)}`;
        });
    }
    ...
    displayUPSResults() {
        ...
        document.getElementById('upsFinalBasic').textContent = `â‚¹${this.formatCurrency(ups.finalBasicPay)}`;
        document.getElementById('upsMonthlyPension').textContent = `â‚¹${this.formatCurrency(ups.monthlyPension)}`;
        document.getElementById('upsLumpSum').textContent = `â‚¹${this.formatCurrency(ups.lumpSum)}`;
        document.getElementById('upsFamilyPension').textContent = `â‚¹${this.formatCurrency(ups.familyPension)}`;
        document.getElementById('upsTotalContribution').textContent = `â‚¹${this.formatCurrency(ups.totalContribution)}`;
    }
    ...
    displayComparison() {
        ...
        document.getElementById('comparisonNPSPension').textContent = `â‚¹${this.formatCurrency(nps.monthlyPension)}`;
        document.getElementById('comparisonUPSPension').textContent = `â‚¹${this.formatCurrency(ups.monthlyPension)}`;
        ...
        document.getElementById('comparisonNPSLumpSum').textContent = `â‚¹${this.formatCurrency(nps.lumpSum)}`;
        document.getElementById('comparisonUPSLumpSum').textContent = `â‚¹${this.formatCurrency(ups.lumpSum)}`;
        ...
        document.getElementById('npsLifetimeValue').textContent = `â‚¹${this.formatCurrency(npsLifetime)}`;
        document.getElementById('upsLifetimeValue').textContent = `â‚¹${this.formatCurrency(upsLifetime)}`;
        document.getElementById('lifetimeWinner').textContent = 
            upsLifetime > npsLifetime ? `UPS (+â‚¹${this.formatCurrency(upsLifetime - npsLifetime)})` : `NPS (+â‚¹${this.formatCurrency(npsLifetime - upsLifetime)})`;
    ...
    createRetirementChart() {
        ...
        const layout = {
            ...
            yaxis: { title: 'Total Value (â‚¹)', tickformat: 'â‚¹,.0f' },
            barmode: 'group',
            font: { family: 'Inter' },
            plot_bgcolor: '#f9fafb',
            paper_bgcolor: '#ffffff',
    ...
    downloadRetirementPDF() {
        ...
        doc.setFontSize(10);
        doc.text(`Final Corpus: â‚¹${this.formatCurrency(nps.finalCorpus)}`, 25, 60);
        doc.text(`Lump Sum: â‚¹${this.formatCurrency(nps.lumpSum)}`, 25, 70);
        doc.text(`Monthly Pension: â‚¹${this.formatCurrency(nps.monthlyPension)}`, 25, 80);

        doc.setFontSize(14);
        doc.text('UPS Benefits:', 20, 100);
        doc.setFontSize(10);
        doc.text(`Monthly Pension: â‚¹${this.formatCurrency(ups.monthlyPension)}`, 25, 110);
        doc.text(`Lump Sum: â‚¹${this.formatCurrency(ups.lumpSum)}`, 25, 120);
        doc.text(`Family Pension: â‚¹${this.formatCurrency(ups.familyPension)}`, 25, 130);
